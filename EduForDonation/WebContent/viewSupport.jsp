		...
		<div class="row">
			...
			<table class="table table-striped"
				...
				<tbody>
					...
					<tr>
						<td>지원부서 및 지원동기</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%=Main.getReasonForApplication().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
					.replaceAll(">", "&gt;").replaceAll("\n", "<br>")%></td>
					</tr>
					<tr>
						<td>자신의 장단점</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%=Main.getProsAndCons().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
					.replaceAll("\n", "<br>")%></td>
					<tr>
						<td>하고싶은 프로그램</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%=Main.getProgramYouWant().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
					.replaceAll("\n", "<br>")%></td>
					</tr>
				</tbody>
			</table>
			<a href="mainsupport.jsp" class="btn btn-primary">목록</a>
			<%
				if (EmailID != null && EmailID.equals(Main.getEmailID())) {
			%>
			<a href="updateSupport.jsp?SupportID=<%=SupportID%>"
				class="btn btn-primary">수정</a> 
				<a onclick="return confirm('해당 지원서를 삭제하시겠습니까?')" href="deleteAction.jsp?SupportID=<%=SupportID%>"
