...
<body>
	<%
		...
		SocSoc Main = new SocSocDAO().getSocSoc(SupportID);
		if (EmailID == null) {
			PrintWriter script = response.getWriter();
			script.println("<script>");
		...
		<div class="row">
			...
			<table class="table table-striped"
				...
				<thead>
					<tr>
						<th colspan="3"
							style="background-color: #eeeeee; text-align: center;">쏙쏙캠프
							신청서 보기</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						<td>부서명</td>
						<td colspan="2" style="min-height: 200px; text-align: left;"><%=Main.getDepartment().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
					.replaceAll("\n", "<br>")%></td>
					</tr>
					<tr>
						<td>사이즈명</td>
			...
			<a href="socsupport.jsp" class="btn btn-primary">목록</a>
			<%
				if (EmailID != null && EmailID.equals(Main.getEmailID())) {
			%>
			<a href="updateSupport2.jsp?SupportID=<%=SupportID%>"
				class="btn btn-primary">수정</a> <a
				onclick="return confirm('해당 신청서를 삭제하시겠습니까?')"
				href="deleteAction2.jsp?SupportID=<%=SupportID%>"
