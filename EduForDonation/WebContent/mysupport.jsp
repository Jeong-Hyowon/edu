	...
	<div class="container">
		<div class="row">
			<table class="table table-striped"
				...
				<tbody>
					<%
						...
						for (int i = 0; i < list.size(); i++) {
							if(session.getAttribute("EmailID").equals(list.get(i).getEmailID())){
							%>
					<tr>
						...
						<td><%=list.get(i).getSupportDate().substring(0, 11)
							+ list.get(i).getSupportDate().substring(11, 13) + "시"
							+ list.get(i).getSupportDate().substring(14, 16) + "분"%></td>
					</tr>
					<%
						}}
				...
				<tbody>
					<%
						...
						ArrayList<Main> list2 = mainDAO.getList2(pageNumber);
						for (int i = 0; i < list2.size(); i++) {
							if(session.getAttribute("EmailID").equals(list2.get(i).getEmailID())){
							%>
					<tr>
						
						<td><a
							href="viewSupport.jsp?SupportID=<%=list2.get(i).getSupportID()%>"><%=list2.get(i).getName().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;")
							.replaceAll(">", "&gt;").replaceAll("\n", "<br>")%></a></td>
						<td><%=list2.get(i).getEmailID()%></td>
						<td><%=list2.get(i).getSupportDate().substring(0, 11)
							+ list2.get(i).getSupportDate().substring(11, 13) + "시"
							+ list2.get(i).getSupportDate().substring(14, 16) + "분"%></td>
					</tr>
					<%
						}}
			...
			<%
				if (pageNumber != 1) {
			%>
			<a href="mysupport.jsp?pageNumber=<%=pageNumber - 1%>"
